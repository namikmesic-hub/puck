/**
 * Source of the runner agent deployed into each environment container at
 * /opt/puck/runner.js. It imports the provider SDKs (installed container-side
 * under /opt/puck/node_modules) and speaks NDJSON over stdio:
 *
 *   in:  {id, op:"turn", provider, prompt, model, resume}
 *        {id, op:"interrupt"}
 *        {id, op:"answer", askId, answers}
 *   out: {id, event:<HarnessEvent>} … {id, session} … {id, done:true, providerSessionId}
 *
 * Execution is organized around the PROVIDERS table below — the container-side
 * mirror of the host Provider interface (src/main/providers). Kept in sync BY
 * HAND: TypeScript cannot reach into this string.
 *
 * Kept as an embedded string (plain CommonJS, no backticks/dollar-brace) so it
 * can be written to a temp file and docker-cp'd with no build step.
 */
